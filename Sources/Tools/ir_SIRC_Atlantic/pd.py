...
_TIME_LC   = 4.918   # leader code, in ms
_TIME_RC   = 11.25  # repeat code, in ms
_TIME_ONE  =  1.631  # one data bit, in ms
_TIME_ZERO =  0.816 # zero data bit, in ms
...
class Ann:
    ...
    LEADER_CODE, DAT, DAT_INV, CMD, CMD_INV, REPEAT_CODE, \
...
class Decoder(srd.Decoder):
    ...
    id = 'ir_nec_atl'
    name = 'IR SIRC ATL'
    longname = 'IR SIRC ATLANTIC'
    desc = 'Atlantic Fujitsu infrared remote control protocol.'
    license = 'gplv2+'
    inputs = ['logic']
    outputs = []
    tags = ['IR']
    ...
    annotation_rows = (
        ...
        ('fields', 'Fields', (Ann.LEADER_CODE, Ann.DAT, Ann.DAT_INV, Ann.CMD, Ann.CMD_INV, Ann.REPEAT_CODE)),
    ...
    def putd(self, data, bit_count):
        ...
        d = {'DATA': Ann.DAT, 'DATA#': Ann.DAT_INV,
             'COMMAND': Ann.CMD, 'COMMAND#': Ann.CMD_INV}
        s = {'DATA': ['DAT', 'A'], 'DATA#': ['DAT#', 'A#'],
    ...
    def data_ok(self, check, want_len):
        ...
        is_ext_addr = self.is_extended and self.state == 'DATA'
        if is_ext_addr:
        ...
        while True:
            ...
            if self.state == 'IDLE':
                ...
                if len(self.data) == 8:
                    ...
                    self.state = 'DATA' if self.is_extended else 'DATA'
            elif self.state == 'DATA#':
                self.handle_bit(width)
                if len(self.data) == 16:
                    self.data_ok(True, 8)
